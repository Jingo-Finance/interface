...
import { CurrencyAmount, Fraction, Percent, Price, Token } from '@jingofi/sdk-core'
import { FeeAmount, Pool, Position, priceToClosestTick, TickMath } from '@jingofi/v3-sdk'
                ...
                {chainId && refund0 && refund1 ? (
                  <ThemedText.DeprecatedBlack fontSize={12}>
                    <Trans>
                      ...
                      {chainId && WRAPPED_NATIVE_CURRENCY[chainId]?.equals(token0) ? 'XPL' : token0.symbol} and{' '}
                      {formatCurrencyAmount(refund1, 4)}{' '}
                      {chainId && WRAPPED_NATIVE_CURRENCY[chainId]?.equals(token1) ? 'XPL' : token1.symbol} will be
